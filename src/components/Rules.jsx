...
const Rules=( { content } ) => {
  ...
  return (
    <div className='relative z-0 pb-10 bg-primary'>
      ...
      <motion.div className='mt-[120px] mb-[5rem] text-center' variants={textVariant()}>
      ...
      >
        ...
        </motion.div>
        {
          content.prizePool.map( ( prizeAmmount, i ) => {
            if ( i===0 )
              return <p>First Prize: <span className='font-bold'>Rs. {prizeAmmount}</span></p>
            else if ( i===1 )
              return <p>Second Prize: <span className='font-bold'>Rs. {prizeAmmount}</span></p>
            else if ( i===2 )
              return <p>Third Prize: <span className='font-bold'>Rs. {prizeAmmount}</span></p>
          } )
        }
      </motion.p>
      }
      ...
      {/* <motion.p
        ...
        className='mt-4 mb-[3rem] px-5 text-secondary text-[17px] max-w-7xl mx-auto leading-[30px]'
      >
        <button disabled className='registerBtn w-[200px] font-bold h-[60px] text-[20px] gradient_color hover:bg-[#6825f7] text-white' onClick={() => navigate( '/register' )} >Register is completed</button>
      </motion.p> */}
